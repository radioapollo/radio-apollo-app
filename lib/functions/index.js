...
// ═══════════════════════════════════════════════════════════════════════════
// adminToggleLike  (admin or studio)
// ═══════════════════════════════════════════════════════════════════════════
//
// Toggles a station like on a chat message. Admin and studio are SEPARATE
// like identities, keyed under distinct map fields:
//   - admin  → likedBy.adminLike
//   - studio → likedBy.studioLike
// So an admin like and a studio like are independent — both can be set at
// once (count reaches 2), and one logging out doesn't remove the other.
// These are also independent from any regular username the admin/studio
// person may have claimed (that lives under likedBy.<username>).
//
// The key names start with a letter, use only permitted characters, and
// don't match Firestore's reserved __.*__ field pattern, so they never
// collide with a real username or get rejected by Firestore.
//
// Callable with a valid admin OR studio session token. The entry in
// `likedBy` is written via Admin SDK, which bypasses the client rules
// that constrain `likes` to change by exactly ±1 — that constraint
// exists for client-direct writes to prevent abuse, and doesn't apply
// when the server is the writer.
...
exports.adminToggleLike = functions.region(REGION).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    // Each role toggles its OWN like cell. This is the whole point of the
    // separate-likes design: admin and studio don't share a like.
    const likeKey = sessionRole === 'studio' ? 'studioLike' : 'adminLike';
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const wasLiked = likedBy[likeKey] === true;
