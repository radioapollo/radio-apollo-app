/* Cloud Functions — Entry point
   ...
   APP CHECK STRATEGY (per-route):
   - claimUsername: STRICT. Username squatting is the high-value attack —
     reject anything without a valid token.
   - userSendMessage: SOFT. App Check failure logs a warning but allows
     the call through with a stricter rate limit. This keeps Xiaomi /
     HyperOS users (whose Play Integrity often fails) able to chat,
     while making spam from non-attested callers expensive.
*/
...
// ═══════════════════════════════════════════════════════════════════════════
// User Send Message — SOFT App Check enforcement
// ═══════════════════════════════════════════════════════════════════════════
//
// App Check failure does not reject the call. Instead:
// 1. The failure is logged for monitoring.
// 2. A stricter rate limit applies (2 per 30s instead of 10 per 60s).
// This keeps Xiaomi / HyperOS users (whose Play Integrity often fails)
// able to chat, while throttling spam scripts hard.
...
exports.userSendMessage = functions.region('europe-west1').https.onRequest(async (req, res) => {
  ...
  try {
    ...
    // Username must exist in the claimed usernames collection. Combined
    // with claimUsername's STRICT App Check, this prevents impersonation
    // of names that were claimed by attested devices, even though we
    // accept calls from non-attested devices.
    ...
    // ── Rate limit ────────────────────────────────────────────────────────
    // Attested calls get the normal 10/min limit.
    // Unattested calls get a 2/30s limit — enough for a real human typing
    // slowly, but a spam script gets throttled hard.
    ...
    await db.collection('chat_messages').add({
      ...
      role: 'user',
      timestamp: admin.firestore.FieldValue.serverTimestamp(),
      // Soft-fail flag so admins can see which messages came in unattested
...
// ═══════════════════════════════════════════════════════════════════════════
// Claim Username — STRICT App Check enforcement
// ═══════════════════════════════════════════════════════════════════════════
//
// Username squatting is a high-value attack: claim "Frank" to impersonate
// the real Frank in chat. We keep this strict so only attested devices
// can claim names. Xiaomi/HyperOS users who can't pass App Check will
// not be able to claim a username — that's the trade-off.
