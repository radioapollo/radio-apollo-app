/* Cloud Functions — Entry point
   ...
   Routes:
   - adminLogin: checks the password against config/admin then config/studio,
     returns { token, role }. One password field, server decides the role.
   - adminSendMessage: posts as "Radio Apollo" (role 'admin'), admin only
   - studioSendMessage: posts as "Studio" (role 'studio'), studio or admin
   ...
   Privileged roles:
   - 'admin'  : full moderation (delete, ban, likes) + posts as "Radio Apollo"
   - 'studio' : posts/replies only, no moderation, posts as "Studio"
...
// ═══════════════════════════════════════════════════════════════════════════
//
// One password field on the client. The server checks the password
// against config/admin first, then config/studio, and returns the role
// it unlocked. Rejects if neither matches.
...
exports.adminLogin = functions.region(REGION).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    let role = null;
    if (await bcrypt.compare(password, adminDoc.data().passwordHash)) {
      role = 'admin';
    } else {
      const studioDoc = await db.collection('config').doc('studio').get();
      if (studioDoc.exists &&
          await bcrypt.compare(password, studioDoc.data().passwordHash)) {
        role = 'studio';
      }
    }
...
// ═══════════════════════════════════════════════════════════════════════════
// adminSendMessage  (role 'admin' → posts as "Radio Apollo")
// ═══════════════════════════════════════════════════════════════════════════
...
exports.adminSendMessage = functions.region(REGION).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const sessionRole = await validateSessionToken(token);
    if (!sessionRole) {
      res.status(401).json({ error: 'Invalid or expired token' });
      return;
    }
    // adminSendMessage is admin-only so the orange "Radio Apollo"
    // identity is exclusively the moderator's. Studio posts go through
    // studioSendMessage.
...
// ═══════════════════════════════════════════════════════════════════════════
// studioSendMessage  (role 'studio' or 'admin' → posts as "Studio")
// ═══════════════════════════════════════════════════════════════════════════
//
// The green "Studio" identity the presenters use on the studio PC. Can
// post and reply like a normal user but has NO moderation powers. An
// admin token is also accepted since admin is strictly more privileged.

exports.studioSendMessage = functions.region(REGION).https.onRequest(async (req, res) => {
  if (preflight(req, res)) return;
  ...
  try {
    ...
    if (sessionRole !== 'studio' && sessionRole !== 'admin') {
      res.status(403).json({ error: 'Geen toegang.' });
      return;
    }
    ...
    console.error('studioSendMessage:', e);
    res.status(500).json({ error: 'Internal error' });
  }
});

// ═══════════════════════════════════════════════════════════════════════════
// adminBanUsername  (admin only)
// ═══════════════════════════════════════════════════════════════════════════
//
...
// ═══════════════════════════════════════════════════════════════════════════
// adminToggleLike  (admin only)
// ═══════════════════════════════════════════════════════════════════════════
//
// Toggles the admin's like on a chat message. The admin like identity is
// separate from any regular username the admin may have claimed — so
// admin Raf can like a message AS the admin while their personal "Raf"
// like (made before becoming admin) also stays in place.
//
// The like is keyed under the map field `likedBy.stationLike`. This is a
// non-username key (no real user would claim "stationLike" as a display
// name, and it doesn't match Firestore's reserved __.*__ field pattern)
// so it never collides with a user's own like or the "Studio" name.
// (not a real username) so it never collides with the green "Studio"
// account's display name or any user's name.
//
// Only callable with a valid admin session token. The entry in `likedBy`
// is written via Admin SDK, which bypasses the client rules that
// constrain `likes` to change by exactly ±1 — that constraint exists for
// client-direct writes to prevent abuse, and doesn't apply when the
// server is the writer.

exports.adminToggleLike = functions.region(REGION).https.onRequest(async (req, res) => {
  if (preflight(req, res)) return;
  ...
  try {
    ...
    // Studio shares the admin (stationLike) like identity, so both admin
    // and studio tokens may toggle this like. Studio still can't reach
    // the actual moderation endpoints (delete/ban) — those stay
    // admin-only.
    if (sessionRole !== 'admin' && sessionRole !== 'studio') {
      res.status(403).json({ error: 'Geen toegang.' });
      return;
    }

    const ref = db.collection('chat_messages').doc(String(messageId));
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const wasLiked = likedBy.stationLike === true;
    ...
    if (e && e.code === 'not_found') {
      res.status(404).json({ error: 'Bericht bestaat niet meer.' });
      return;
    }
    console.error('adminToggleLike:', e);
    res.status(500).json({ error: 'Internal error' });
  }
});

// ═══════════════════════════════════════════════════════════════════════════
// adminUpdateReport  (admin only)
// ═══════════════════════════════════════════════════════════════════════════
//
...
//
// Studio messages (role 'studio') are treated like admin messages for
// notification purposes — a studio post is still "the station talking",
// so it goes to the studio_messages topic.
...
exports.onChatMessageCreated = functions
  ...
  .onCreate(async (snap, _context) => {
    ...
    if (data.role === 'admin' || data.role === 'studio') {
