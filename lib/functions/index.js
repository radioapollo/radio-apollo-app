...
// ═══════════════════════════════════════════════════════════════════════════
//
// Toggles a "Studio" like on a chat message. The admin identity is
// separate from any regular username the admin may have claimed —
// so admin Raf can like a message AS Studio while their personal
// "Raf" like (made before becoming admin) also stays in place.
//
// Only callable with a valid admin session token. The Studio entry
// in `likedBy` is written via Admin SDK, which bypasses the client
// rules that constrain `likes` to change by exactly ±1 — that
// constraint exists for client-direct writes to prevent abuse, and
// doesn't apply when the server is the writer.
...
exports.adminToggleLike = functions.region(REGION).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const wasLiked = likedBy.Studio === true;
