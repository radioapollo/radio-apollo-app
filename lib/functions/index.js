...
exports.adminLogin = functions.region('europe-west1').https.onRequest(async (req, res) => {
  ...
  try {
    ...
    // Use the trusted proxy-derived IP, NOT any client-supplied header.
...
exports.userSendMessage = functions.region('europe-west1').https.onRequest(async (req, res) => {
  ...
  try {
    // ── App Check: the request must come from a genuine build of the app.
    ...
    if (trimmedUsername.length < 2 || trimmedUsername.length > 20) {
      res.status(400).json({ error: 'Gebruikersnaam moet tussen 2 en 20 tekens zijn.' });
      return;
    }
    ...
    // Require the username to exist in the claimed usernames collection.
    // This doesn't prove ownership on its own — App Check is what does that —
    // but it blocks impersonation of names nobody has claimed.
    const claimDoc = await db.collection('usernames').doc(trimmedUsername.toLowerCase()).get();
    if (!claimDoc.exists) {
      res.status(400).json({ error: 'Onbekende gebruikersnaam.' });
      return;
    }
 
    // Rate limit bucket: IP + username combined. Username alone is rotateable;
    // IP alone punishes shared networks too hard. The combination works well.
    const ip = getClientIp(req);
    ...
    if (filterResult.isSevere) {
      res.status(400).json({ error: 'Dit bericht kan niet worden verzonden. Blijf vriendelijk.' });
      return;
    }
...
//
// Atomically creates a usernames/{lowerName} document. Returns 409 if the
// name is already taken, 400 for invalid input, 401 if App Check fails.
