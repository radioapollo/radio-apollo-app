/* Cloud Functions — Notifications
   ...
   ─────────────────────────────
   For chat_activity, we include `username` in the message data so the
   client can drop the notification if the sender is the local user.
   See _displayForegroundMessage in notification_service.dart.
 ...
 *
 * Optional `extraData` is merged into the message data — used for
 * passing the sender's username so the client can suppress a user's
 * own messages, and similar lightweight metadata.
 */
async function sendToTopic({ topic, channelId, title, body, category, extraData }) {
  const data = { category };
 ...
 *
 * Includes the sender's username in the data payload so the client
 * can suppress notifications for messages the local user sent
 * themselves (otherwise you'd get pinged every time you hit send).
 */
 ...
 * dates ("3 mei 2026") and multi-day ranges ("30/31 mei 2026").
 *
