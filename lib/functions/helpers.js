...
// ─── Trusted client IP ──────────────────────────────────────────────────────
//
// Cloud Functions sits behind Google's front-end proxies which set
// `req.ip` based on the trusted proxy chain. We DO NOT fall back to
// `req.headers['x-forwarded-for']` because that header is attacker-controlled
// and can be set to any value to defeat rate limiting.
...
//
// Verifies the X-Firebase-AppCheck header. Returns true if the request
// comes from a genuine, attested build of the app. Returns false otherwise
// (missing header, expired token, wrong project, etc.).
...
//
// Returns a plain boolean: true if the call may proceed, false if it has
// exceeded USER_MSG_LIMIT within USER_MSG_WINDOW_MS for this bucket key.
